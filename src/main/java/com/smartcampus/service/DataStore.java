...
public class DataStore {
   // ── Singleton boilerplate ─────────────────────────────────────────────────
    ...
    // ── Constructor: seed some sample data ───────────────────────────────────
    private DataStore() {
        // Seed rooms
        ...
        // Register sensor IDs in their rooms
    ...
    /**
     * Atomically check if room exists and delete it only if it has no sensors.
     * Returns true on successful deletion, false if room does not exist.
     * Throws IllegalStateException if sensors are still assigned.
     */
    public synchronized boolean deleteRoom(String id) {
        ...
        if (!room.getSensorIds().isEmpty()) {
            throw new IllegalStateException("ROOM_NOT_EMPTY");
        }
        rooms.remove(id);
        return true;
    }
    ...
    /**
     * Atomically add a sensor and register its ID in the parent room.
     * Throws IllegalArgumentException if the roomId does not exist.
     */
    public synchronized void addSensor(Sensor sensor) {
        Room room = rooms.get(sensor.getRoomId());
        if (room == null) {
            throw new IllegalArgumentException("LINKED_RESOURCE_NOT_FOUND");
        }
    ...
    /**
     * Returns the reading list for a sensor, creating it if absent.
     */
    ...
    /**
     * Atomically append a reading and update the parent sensor's currentValue.
     */
